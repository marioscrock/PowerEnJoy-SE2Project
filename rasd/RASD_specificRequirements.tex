	...
	\begin{description}
		...
		\item \ref{goal:maintenanceDone}\ Provide the maintenance service with a way to notify the system when a car is available again
			\begin{enumerate}[resume*]
   				...
   				\item When the system receives a request to set the state of a car as
   				\emph{Available} through the API, before to process the request the system must
   				check if the car is locked and its state is set as \emph{Not Available}
  			\end{enumerate}
