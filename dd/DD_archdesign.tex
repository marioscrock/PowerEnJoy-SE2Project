...
\paragraph{Safe areas}Safe areas are also provided as XML file according to the \emph{GPX GPS Exchange Format} \cite{gpx} and they are composed by a closed broken line.\todo{cite ref}
...
In the \autoref{tbl:mappingGoalsOnComponent} is presented a mapping correspondence between the RASD requirements related to each goal and the components identified in the server component diagram. \todo{check} \\
...
\end{figure}
\paragraph{Mapping of Server components on architecture:} to clarify at a finer level how the Server components are mapped in the four tier layered architecture the following diagram represent the Server components highlighted in the same color of the tier in the previous diagram:
\begin{itemize}
	\item 
\end{itemize}
\clearpage
...
BlaBla \todo{description}
\begin{figure}[h!]
	\centering
	\includegraphics[angle=90,width=0.78\linewidth]{sequenceDiagrams/ReserveCar}
	\caption{
...
The \emph{UserAppInt} interface is responsible for communications between the user application and the related web server.
...
\begin{itemize}
	...
	\item get the latitude and longitude of a given location
\end{itemize}
