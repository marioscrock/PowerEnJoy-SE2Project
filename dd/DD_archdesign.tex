...
\paragraph{CcUserInfoManager:}
\paragraph{UserInfoManager:}
\paragraph{BanUnbanManager:}
\clearpage
...
\paragraph{Interactions not represented}When a car has to be tagged back as \mbox{Available} the \emph{UnavailableCarManager} component takes care of verifying if the car is associated with an open failure, if the doors are locked and all the others checks defined in the RASD.
