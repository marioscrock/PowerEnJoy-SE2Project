...
\begin{itemize}
	\item \textbf{Register} to the \emph{PowerEnjoySystem}: to be processed by the system only requires users data
\end{itemize}

\paragraph{Registered user}
\begin{itemize}
	\item \textbf{Authenticate} to the \emph{PowerEnjoySystem}: to be processed by the system only requires users data
	\item \textbf{Edit user info}: to be processed by the system only requires users data
	...
	\item \textbf{Reservation expired} (triggered by reservation timer): to be processed by the system it requires active reservations info and payments data to instantiate a new payment record
	\item \textbf{Unlock a car}: to be processed by the system it requires cars info and access to rents data to instantiate a new rent record
...
\begin{itemize}
	\item \textbf{Ban or enable a user} to be processed by the system it only requires users data
	\item \textbf{Tag a car as \emph{Not available}} to be processed by the system it only requires cars data
\end{itemize}
...
\end{itemize} 
 ...
 $$\textrm{Lower bound \textbf{SLOC}} = (\sum_{i \in I} \sum_{j \in i} w_{ij}) \cdot factor$$
 
  $$\textrm{Upper Bound \textbf{SLOC}} = (\sum_{i \in I} \sum_{j \in i} w_{ij}) \cdot factor$$