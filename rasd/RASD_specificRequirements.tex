	...
	\begin{description}
		...
		\item \ref{goal:maintenanceDone}\ Provide the maintenance office with a way to notify the system when a car is available again
  		...
  		\begin{enumerate}[resume*]
  			...
  			service
   			\item The system must offer an restricted access API to set the state of a specific car as
   			\emph{Not Available}	
  		\end{enumerate}
  		...
  		prevent them from reserving or using other cars, and enable them to use the service again
  			\begin{enumerate}[resume*]
  				...
  				history \todo{?}
  				\item The system must offer an restricted access API that allows to manage payments
  				of a user \todo{?}
   				\item The system must offer an restricted access API that allows to set the state of a
