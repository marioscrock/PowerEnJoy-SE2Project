		...
		\item \ref{goal:maintenanceDone}\ Provide the maintenance service with a way to notify the system when a car is available again
  		...
  		\item \ref{goal:reservationFee}\ Charge the user for 1\euro\ in case he hasn't used the car he reserved after an hour from such reservation
  			\begin{enumerate}[resume*]
   			...
   			\end{enumerate}
  		\item \ref{goal:completeRent}\ Allow a user to perform a complete rent: reserving a car, using it and leaving it terminating the rent in a safe area, concluding successfully the payment procedure related to the aforementioned rent
  			\begin{enumerate}[resume*]
  				...
  				\item The system must be able to unlock a car when the position of the user paired	with the \emph{Reserved} state of the car is at most 5 meters away from the position the car\todo{incluso in quello che dice che deve essere in grado di chiudere e aprire la macchina? (Aggiungerei solo che l'utente deve essere in grado di comunicare al sistema che si trova a meno di 5 metri dalla macchina)}
  				...
  				\item \todo{Va tolto?} The system must be able to know the state of all charging stations: how many
  				cars are plugged to the station
  				\item The system must be able to know if a car is plugged in a specific charging
  				station
  				\item \todo{Va tolto?} The system must be able to uniquely identify a charging station and the related
  				plugs
   			\end{enumerate}
  		\item \ref{goal:calculateCost}\ Calculate and charge the user for the correct amount of money he has to pay for his last ride, also considering the various discounts and fees applicable based on the ride
  			\begin{enumerate}[resume*]
  				...
  				one passengers into the car, the system must apply a 10\% discount on the cost of the current to the user paired with the
  				\emph{In Use} state of the aforementioned car
  				the current ride
