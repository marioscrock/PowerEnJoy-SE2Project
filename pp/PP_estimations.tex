...
\textbf{Notes} Some data stored by the system are retrieved from cars trough the provided API primitives, these data represent a cache copy of the actual data of the car which instead are considered as an EIF. 
...
\paragraph{Notes: Map} The system relies on data about maps of the GIS API but needs only a map reference to allow the user client to show a map centred in a given position, with a given zoom and a set of pointer and areas (data managed by the system). So it is not considered as an EIF.
...
\begin{itemize}
	\item register to the \emph{PowerEnjoySystem}
\end{itemize}

\paragraph{Registered user}
\begin{itemize}
	\item authenticate to the \emph{PowerEnjoySystem}
	\item see available cars
	\item reserve a car (eventually with money saving option)
	\item unlock a car
	\item edit user info
\end{itemize}
